...
from .forms import ManagerForm, ServTypeForm, SpokesmanForm, ClientForm, ServiceForm, ServiceFormFormSet
from .utils import *
...
class ClientAddServiceView(View):
    ...
    def get(self, request, pk):
        client = Client.objects.get(id=pk)
        form = ServiceFormFormSet(instance=client)
        return render(request, "clilib/clients/clients_services_add.html", {"serviceformset": form, "client": client})

    def post(self, request, pk):
        client = Client.objects.get(id=pk)
        bound_form = ServiceFormFormSet(request.POST, instance=client)
        if bound_form.is_valid():
            new_obj = bound_form.save()
            return redirect('client_detail_url', pk = pk)
        return render(request, "clilib/clients/clients_services_add.html", {"serviceformset": bound_form, "client": client})
